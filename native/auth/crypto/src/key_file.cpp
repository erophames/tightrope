...
bool write_key_file(
    ...
    const auto salt_hex = core::text::hex_encode(std::span<const std::uint8_t>(salt.data(), salt.size()));
    const auto nonce_hex = core::text::hex_encode(std::span<const std::uint8_t>(nonce.data(), nonce.size()));
    const auto cipher_hex =
        core::text::hex_encode(std::span<const std::uint8_t>(ciphertext.data(), ciphertext.size()));
...
std::optional<SecretKey> read_key_file(
    ...
    std::array<unsigned char, crypto_pwhash_SALTBYTES> salt{};
    std::array<unsigned char, crypto_secretbox_NONCEBYTES> nonce{};
    if (!core::text::hex_decode(salt_hex, std::span<std::uint8_t>(salt.data(), salt.size()))) {
        return fail_with_optional_error(error, "invalid key file salt");
    }
    if (!core::text::hex_decode(nonce_hex, std::span<std::uint8_t>(nonce.data(), nonce.size()))) {
        return fail_with_optional_error(error, "invalid key file nonce");
    }

    const auto decoded_cipher = core::text::hex_decode(cipher_hex);
    if (!decoded_cipher.has_value()) {
        return fail_with_optional_error(error, "invalid key file payload");
    }
    std::vector<unsigned char> ciphertext(decoded_cipher->begin(), decoded_cipher->end());
