...
bool is_valid_host_port(const HostPortEndpoint& endpoint) {
...
std::optional<HostPortEndpoint> parse_host_port(const std::string_view address) {
    if (address.empty()) {
        return std::nullopt;
    }
    ...
    HostPortEndpoint endpoint{
        .host = std::move(host),
        .port = static_cast<std::uint16_t>(port_u32),
    };
    if (!is_valid_host_port(endpoint)) {
        return std::nullopt;
    }
    return endpoint;
}

std::string host_port_to_string(const HostPortEndpoint& endpoint) {
