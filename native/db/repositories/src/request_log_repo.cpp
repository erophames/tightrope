...
constexpr const char* kAddRoutingStrategySql = "ALTER TABLE request_logs ADD COLUMN routing_strategy TEXT;";
constexpr const char* kAddStickySql = "ALTER TABLE request_logs ADD COLUMN sticky INTEGER NOT NULL DEFAULT 0;";
constexpr const char* kAddRoutingScoreSql = "ALTER TABLE request_logs ADD COLUMN routing_score REAL;";
constexpr const char* kAddTotalCostSql = "ALTER TABLE request_logs ADD COLUMN total_cost REAL NOT NULL DEFAULT 0;";
constexpr const char* kAddLatencyMsSql = "ALTER TABLE request_logs ADD COLUMN latency_ms INTEGER;";
constexpr const char* kAddTotalTokensSql = "ALTER TABLE request_logs ADD COLUMN total_tokens INTEGER;";
...
bool ensure_schema(SQLite::Database& db) noexcept {
    ...
    if (!sqlite_repo_utils::ensure_column(db, "request_logs", "account_id", kAddAccountIdSql) ||
        ...
        !sqlite_repo_utils::ensure_column(db, "request_logs", "routing_strategy", kAddRoutingStrategySql) ||
        !sqlite_repo_utils::ensure_column(db, "request_logs", "sticky", kAddStickySql) ||
        !sqlite_repo_utils::ensure_column(db, "request_logs", "routing_score", kAddRoutingScoreSql) ||
        !sqlite_repo_utils::ensure_column(db, "request_logs", "total_cost", kAddTotalCostSql) ||
        !sqlite_repo_utils::ensure_column(db, "request_logs", "latency_ms", kAddLatencyMsSql) ||
        !sqlite_repo_utils::ensure_column(db, "request_logs", "total_tokens", kAddTotalTokensSql)) {
        return false;
    }
...
INSERT INTO request_logs (
    ...
    try {
        ...
        if (!sqlite_repo_utils::bind_optional_text(stmt, 5, log.model) ||
            ...
            !sqlite_repo_utils::bind_optional_text(stmt, 8, log.routing_strategy)) {
            return false;
        }
        stmt.bind(9, log.sticky ? 1 : 0);
        if (log.routing_score.has_value()) {
            stmt.bind(10, *log.routing_score);
        } else {
            stmt.bind(10);
        }
        stmt.bind(11, log.total_cost);
        if (log.latency_ms.has_value()) {
            stmt.bind(12, *log.latency_ms);
        } else {
            stmt.bind(12);
        }
        if (log.total_tokens.has_value()) {
            stmt.bind(13, *log.total_tokens);
        } else {
            stmt.bind(13);
        }
        return stmt.exec() > 0;
...
SELECT id, account_id, path, method, status_code, model, requested_at, error_code, transport, routing_strategy, sticky, total_cost
     , routing_score, latency_ms, total_tokens
FROM request_logs
    ...
    try {
        ...
        while (stmt.executeStep()) {
            ...
            if (!stmt.getColumn(9).isNull()) {
                record.routing_strategy = stmt.getColumn(9).getString();
            }
            if (!stmt.getColumn(10).isNull()) {
                record.sticky = stmt.getColumn(10).getInt() != 0;
            }
            if (!stmt.getColumn(11).isNull()) {
                record.total_cost = stmt.getColumn(11).getDouble();
            }
            if (!stmt.getColumn(12).isNull()) {
                record.routing_score = stmt.getColumn(12).getDouble();
            }
            if (!stmt.getColumn(13).isNull()) {
                record.latency_ms = stmt.getColumn(13).getInt64();
            }
            if (!stmt.getColumn(14).isNull()) {
                record.total_tokens = stmt.getColumn(14).getInt64();
            }
