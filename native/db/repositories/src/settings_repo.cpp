...
std::size_t count_locked_routing_account_ids(std::string_view raw) {
    std::size_t count = 0;
...
CREATE TABLE IF NOT EXISTS dashboard_settings (
    ...
    routing_strategy TEXT NOT NULL DEFAULT 'weighted_round_robin',
    strict_lock_pool_continuations INTEGER NOT NULL DEFAULT 0,
    locked_routing_account_ids TEXT NOT NULL DEFAULT '',
...
SET
    ...
    routing_strategy = COALESCE(routing_strategy, 'weighted_round_robin'),
    strict_lock_pool_continuations = COALESCE(strict_lock_pool_continuations, 0),
    locked_routing_account_ids = COALESCE(locked_routing_account_ids, ''),
...
bool ensure_schema(SQLite::Database& db) noexcept {
    ...
    if (!sqlite_repo_utils::ensure_column(
        ...
        !sqlite_repo_utils::ensure_column(
            ...
            "ALTER TABLE dashboard_settings ADD COLUMN strict_lock_pool_continuations INTEGER NOT NULL DEFAULT 0;"
        ) ||
        !sqlite_repo_utils::ensure_column(
            db,
            "dashboard_settings",
            "locked_routing_account_ids",
            "ALTER TABLE dashboard_settings ADD COLUMN locked_routing_account_ids TEXT NOT NULL DEFAULT '';"
        ) ||
        !sqlite_repo_utils::ensure_column(
            db,
            "dashboard_settings",
...
LIMIT 1;
    ...
    try {
        ...
        record.strict_lock_pool_continuations = to_bool(stmt, 5);
        if (!stmt.getColumn(6).isNull()) {
            record.openai_cache_affinity_max_age_seconds = stmt.getColumn(6).getInt64();
        }
        record.import_without_overwrite = to_bool(stmt, 7);
        record.totp_required_on_login = to_bool(stmt, 8);
        record.api_key_auth_enabled = to_bool(stmt, 9);
        if (!stmt.getColumn(10).isNull()) {
            record.routing_headroom_weight_primary = stmt.getColumn(10).getDouble();
        }
        if (!stmt.getColumn(11).isNull()) {
            record.routing_headroom_weight_secondary = stmt.getColumn(11).getDouble();
        }
        if (!stmt.getColumn(12).isNull()) {
            record.routing_score_alpha = stmt.getColumn(12).getDouble();
        }
        if (!stmt.getColumn(13).isNull()) {
            record.routing_score_beta = stmt.getColumn(13).getDouble();
        }
        if (!stmt.getColumn(14).isNull()) {
            record.routing_score_gamma = stmt.getColumn(14).getDouble();
        }
        if (!stmt.getColumn(15).isNull()) {
            record.routing_score_delta = stmt.getColumn(15).getDouble();
        }
        if (!stmt.getColumn(16).isNull()) {
            record.routing_score_zeta = stmt.getColumn(16).getDouble();
        }
        if (!stmt.getColumn(17).isNull()) {
            record.routing_score_eta = stmt.getColumn(17).getDouble();
        }
        if (!stmt.getColumn(18).isNull()) {
            record.routing_success_rate_rho = stmt.getColumn(18).getDouble();
        }
        if (const auto value = sqlite_repo_utils::optional_text(stmt.getColumn(19)); value.has_value()) {
            record.routing_plan_model_pricing_usd_per_million = *value;
        }
        if (const auto sync_cluster_name = sqlite_repo_utils::optional_text(stmt.getColumn(20)); sync_cluster_name.has_value()) {
            record.sync_cluster_name = *sync_cluster_name;
        }
        if (!stmt.getColumn(21).isNull()) {
            record.sync_site_id = stmt.getColumn(21).getInt64();
        }
        if (!stmt.getColumn(22).isNull()) {
            record.sync_port = stmt.getColumn(22).getInt64();
        }
        record.sync_discovery_enabled = to_bool(stmt, 23);
        if (!stmt.getColumn(24).isNull()) {
            record.sync_interval_seconds = stmt.getColumn(24).getInt64();
        }
        if (const auto sync_conflict_resolution = sqlite_repo_utils::optional_text(stmt.getColumn(25));
            sync_conflict_resolution.has_value()) {
            record.sync_conflict_resolution = *sync_conflict_resolution;
        }
        if (!stmt.getColumn(26).isNull()) {
            record.sync_journal_retention_days = stmt.getColumn(26).getInt64();
        }
        record.sync_tls_enabled = to_bool(stmt, 27);
        record.sync_require_handshake_auth = to_bool(stmt, 28);
        if (const auto value = sqlite_repo_utils::optional_text(stmt.getColumn(29)); value.has_value()) {
            record.sync_cluster_shared_secret = *value;
        }
        record.sync_tls_verify_peer = to_bool(stmt, 30);
        ...
        if (const auto value = sqlite_repo_utils::optional_text(stmt.getColumn(34)); value.has_value()) {
            record.sync_tls_pinned_peer_certificate_sha256 = *value;
        }
        if (!stmt.getColumn(35).isNull()) {
            record.sync_schema_version = stmt.getColumn(35).getInt64();
        }
        if (!stmt.getColumn(36).isNull()) {
            record.sync_min_supported_schema_version = stmt.getColumn(36).getInt64();
        }
        record.sync_allow_schema_downgrade = to_bool(stmt, 37);
        record.sync_peer_probe_enabled = to_bool(stmt, 38);
        if (!stmt.getColumn(39).isNull()) {
            record.sync_peer_probe_interval_ms = stmt.getColumn(39).getInt64();
        }
        if (!stmt.getColumn(40).isNull()) {
            record.sync_peer_probe_timeout_ms = stmt.getColumn(40).getInt64();
        }
        if (!stmt.getColumn(41).isNull()) {
            record.sync_peer_probe_max_per_refresh = stmt.getColumn(41).getInt64();
        }
        record.sync_peer_probe_fail_closed = to_bool(stmt, 42);
        if (!stmt.getColumn(43).isNull()) {
            record.sync_peer_probe_fail_closed_failures = stmt.getColumn(43).getInt64();
        }
        if (const auto connect_address = sqlite_repo_utils::optional_text(stmt.getColumn(44)); connect_address.has_value()) {
            record.connect_address = *connect_address;
        }
        record.password_hash = sqlite_repo_utils::optional_text(stmt.getColumn(45));
        record.totp_secret = sqlite_repo_utils::optional_text(stmt.getColumn(46));
        record.totp_last_verified_step = sqlite_repo_utils::optional_i64(stmt.getColumn(47));
        if (const auto locked = sqlite_repo_utils::optional_text(stmt.getColumn(48)); locked.has_value()) {
            record.locked_routing_account_ids = *locked;
        }
        return record;
    } catch (...) {
        return std::nullopt;
...
std::optional<DashboardSettingsRecord> update_dashboard_settings(sqlite3* db, const DashboardSettingsPatch& patch) noexcept {
    ...
    if (patch.strict_lock_pool_continuations.has_value()) {
        next.strict_lock_pool_continuations = *patch.strict_lock_pool_continuations;
    }
    if (patch.locked_routing_account_ids.has_value()) {
        next.locked_routing_account_ids = *patch.locked_routing_account_ids;
    }
...
SET
    ...
    routing_strategy = ?5,
    strict_lock_pool_continuations = ?6,
    openai_cache_affinity_max_age_seconds = ?7,
    import_without_overwrite = ?8,
    totp_required_on_login = ?9,
    api_key_auth_enabled = ?10,
    routing_headroom_weight_primary = ?11,
    routing_headroom_weight_secondary = ?12,
    routing_score_alpha = ?13,
    routing_score_beta = ?14,
    routing_score_gamma = ?15,
    routing_score_delta = ?16,
    routing_score_zeta = ?17,
    routing_score_eta = ?18,
    routing_success_rate_rho = ?19,
    routing_plan_model_pricing_usd_per_million = ?20,
    sync_cluster_name = ?21,
    sync_site_id = ?22,
    sync_port = ?23,
    sync_discovery_enabled = ?24,
    sync_interval_seconds = ?25,
    sync_conflict_resolution = ?26,
    sync_journal_retention_days = ?27,
    sync_tls_enabled = ?28,
    sync_require_handshake_auth = ?29,
    sync_cluster_shared_secret = ?30,
    sync_tls_verify_peer = ?31,
    sync_tls_ca_certificate_path = ?32,
    sync_tls_certificate_chain_path = ?33,
    sync_tls_private_key_path = ?34,
    sync_tls_pinned_peer_certificate_sha256 = ?35,
    sync_schema_version = ?36,
    sync_min_supported_schema_version = ?37,
    sync_allow_schema_downgrade = ?38,
    sync_peer_probe_enabled = ?39,
    sync_peer_probe_interval_ms = ?40,
    sync_peer_probe_timeout_ms = ?41,
    sync_peer_probe_max_per_refresh = ?42,
    sync_peer_probe_fail_closed = ?43,
    sync_peer_probe_fail_closed_failures = ?44,
    connect_address = ?45,
    dashboard_password_hash = ?46,
    dashboard_totp_secret = ?47,
    totp_last_verified_step = ?48,
    locked_routing_account_ids = ?49,
    updated_at = datetime('now')
WHERE id = 1;
)SQL";
    constexpr const char* kClearAllRoutingPinsSql = R"SQL(
UPDATE accounts
SET routing_pinned = 0,
    updated_at = datetime('now')
WHERE routing_pinned != 0;
)SQL";
    const bool lock_pool_active = count_locked_routing_account_ids(next.locked_routing_account_ids) >= 2;
    if (lock_pool_active && !ensure_accounts_schema(db)) {
        return std::nullopt;
    }
    ...
    try {
        ...
        try {
            ...
            stmt.bind(6, next.strict_lock_pool_continuations ? 1 : 0);
            stmt.bind(7, next.openai_cache_affinity_max_age_seconds);
            stmt.bind(8, next.import_without_overwrite ? 1 : 0);
            stmt.bind(9, next.totp_required_on_login ? 1 : 0);
            stmt.bind(10, next.api_key_auth_enabled ? 1 : 0);
            stmt.bind(11, next.routing_headroom_weight_primary);
            stmt.bind(12, next.routing_headroom_weight_secondary);
            stmt.bind(13, next.routing_score_alpha);
            stmt.bind(14, next.routing_score_beta);
            stmt.bind(15, next.routing_score_gamma);
            stmt.bind(16, next.routing_score_delta);
            stmt.bind(17, next.routing_score_zeta);
            stmt.bind(18, next.routing_score_eta);
            stmt.bind(19, next.routing_success_rate_rho);
            stmt.bind(20, next.routing_plan_model_pricing_usd_per_million);
            stmt.bind(21, next.sync_cluster_name);
            stmt.bind(22, next.sync_site_id);
            stmt.bind(23, next.sync_port);
            stmt.bind(24, next.sync_discovery_enabled ? 1 : 0);
            stmt.bind(25, next.sync_interval_seconds);
            stmt.bind(26, next.sync_conflict_resolution);
            stmt.bind(27, next.sync_journal_retention_days);
            stmt.bind(28, next.sync_tls_enabled ? 1 : 0);
            stmt.bind(29, next.sync_require_handshake_auth ? 1 : 0);
            stmt.bind(30, next.sync_cluster_shared_secret);
            stmt.bind(31, next.sync_tls_verify_peer ? 1 : 0);
            stmt.bind(32, next.sync_tls_ca_certificate_path);
            stmt.bind(33, next.sync_tls_certificate_chain_path);
            stmt.bind(34, next.sync_tls_private_key_path);
            stmt.bind(35, next.sync_tls_pinned_peer_certificate_sha256);
            stmt.bind(36, next.sync_schema_version);
            stmt.bind(37, next.sync_min_supported_schema_version);
            stmt.bind(38, next.sync_allow_schema_downgrade ? 1 : 0);
            stmt.bind(39, next.sync_peer_probe_enabled ? 1 : 0);
            stmt.bind(40, next.sync_peer_probe_interval_ms);
            stmt.bind(41, next.sync_peer_probe_timeout_ms);
            stmt.bind(42, next.sync_peer_probe_max_per_refresh);
            stmt.bind(43, next.sync_peer_probe_fail_closed ? 1 : 0);
            stmt.bind(44, next.sync_peer_probe_fail_closed_failures);
            stmt.bind(45, next.connect_address);
            if (!sqlite_repo_utils::bind_optional_text(stmt, 46, next.password_hash) ||
                !sqlite_repo_utils::bind_optional_text(stmt, 47, next.totp_secret)) {
            ...
            if (!next.totp_last_verified_step.has_value()) {
                ...
                stmt.bind(48, *next.totp_last_verified_step);
            }
            stmt.bind(49, next.locked_routing_account_ids);
            (void)stmt.exec();

            if (lock_pool_active) {
                SQLite::Statement clear_pins_stmt(*handle.db, kClearAllRoutingPinsSql);
                (void)clear_pins_stmt.exec();
            }
