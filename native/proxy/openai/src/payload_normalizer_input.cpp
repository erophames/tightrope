...
Json text_part(const std::string_view type, const std::string& text) {
...
bool is_assistant_role(const std::string_view role) {
    return role == "assistant";
}

bool is_input_text_role(const std::string_view role) {
    return role == "user" || role == "system" || role == "developer";
}

std::string_view text_part_type_for_role(const std::string_view role) {
    return is_assistant_role(role) ? std::string_view("output_text") : std::string_view("input_text");
}
...
Json normalize_message_content_for_role(const Json& content, const std::string_view role) {
    const auto normalized_part_type = text_part_type_for_role(role);
    if (content.is_string()) {
        Json normalized = JsonArray{};
        normalized.get_array().push_back(text_part(normalized_part_type, content.get_string()));
        return normalized;
    }
    if (content.is_array()) {
        Json normalized = JsonArray{};
        ...
        for (const auto& part : content.get_array()) {
            if (part.is_string()) {
                parts.push_back(text_part(normalized_part_type, part.get_string()));
                continue;
            }
            const auto text = extract_text_content_part(part, kAssistantTextPartTypes);
            if (text.has_value()) {
                parts.push_back(text_part(normalized_part_type, *text));
            } else {
    ...
    if (content.is_object()) {
        ...
        normalized.get_array().push_back(text.has_value() ? text_part(normalized_part_type, *text) : content);
        return normalized;
    }
    return content;
}
...
Json normalize_role_input_item(const Json& item) {
    ...
    if (is_assistant_role(*role) || is_input_text_role(*role)) {
        ...
        if (content_it != normalized_object.end()) {
            content_it->second = normalize_message_content_for_role(content_it->second, *role);
        }
        return normalized;
    }
    if (*role != "tool") {
    ...
    if (output_source == nullptr || output_source->is_null()) {
        normalized["output"] = "";
    } else {
        normalized["output"] = *output_source;
    }
    return normalized;
}
...
void normalize_request_base_fields(JsonObject& payload, const bool compact_mode) {
    ...
    if (!compact_mode) {
        auto stream_it = payload.find("stream");
        if (stream_it == payload.end()) {
            payload["stream"] = true;
            stream_it = payload.find("stream");
        }
        if (stream_it->second.is_null()) {
            stream_it->second = true;
        } else if (!stream_it->second.is_boolean()) {
            throw std::runtime_error("stream must be a boolean");
        }

        auto parallel_it = payload.find("parallel_tool_calls");
        if (parallel_it == payload.end()) {
            payload["parallel_tool_calls"] = false;
            parallel_it = payload.find("parallel_tool_calls");
        }
        if (parallel_it->second.is_null()) {
            parallel_it->second = false;
        } else if (!parallel_it->second.is_boolean()) {
            throw std::runtime_error("parallel_tool_calls must be a boolean");
        }

        if (!has_field(payload, "tool_choice")) {
