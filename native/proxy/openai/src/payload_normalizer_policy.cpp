...
void validate_and_normalize_store(JsonObject& payload) {
    ...
    if (!it->second.is_boolean()) {
        throw std::runtime_error("store must be a boolean");
    }
}

void validate_and_normalize_include(JsonObject& payload) {
    auto include_it = payload.find("include");
    if (include_it == payload.end()) {
        return;
    }
    if (include_it->second.is_null()) {
        include_it->second = JsonArray{};
        return;
    }
    ...
    for (const auto& value : include_it->second.get_array()) {
        if (!value.is_string()) {
            throw std::runtime_error("include values must be strings");
        }
    }
}
...
void validate_and_normalize_tools(JsonObject& payload) {
    ...
    if (tools_it->second.is_null()) {
        tools_it->second = JsonArray{};
        return;
    }
