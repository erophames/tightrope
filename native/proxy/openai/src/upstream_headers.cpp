...
constexpr std::array<std::string_view, 9> kIgnoredInboundHeaders = {
    "authorization",
    "chatgpt-account-id",
    "content-encoding",
    "content-length",
...
constexpr std::string_view kResponsesWebsocketBetaToken = "responses_websockets=2026-02-06";
...
void erase_header_case_insensitive(HeaderMap& headers, const std::string_view name_lower) {
    for (auto it = headers.begin(); it != headers.end();) {
        if (core::text::to_lower_ascii(it->first) == name_lower) {
            it = headers.erase(it);
            continue;
        }
        ++it;
...
std::vector<std::string> split_csv_tokens(const std::string_view value) {
...
void ensure_responses_websocket_beta_header(HeaderMap& headers) {
    std::string header_key = "openai-beta";
    std::string header_value;

    for (auto it = headers.begin(); it != headers.end();) {
        if (core::text::to_lower_ascii(it->first) != "openai-beta") {
            ++it;
            continue;
        }
    ...
    auto beta_tokens = split_csv_tokens(header_value);
    const auto has_required = std::any_of(beta_tokens.begin(), beta_tokens.end(), [](const std::string& token) {
        return core::text::equals_case_insensitive(token, kResponsesWebsocketBetaToken);
    });
    if (!has_required) {
        beta_tokens.emplace_back(kResponsesWebsocketBetaToken);
    }

    std::string merged;
    for (std::size_t i = 0; i < beta_tokens.size(); ++i) {
        if (i > 0) {
            merged += ", ";
        }
        merged += beta_tokens[i];
    }
    headers[header_key] = merged;
}
...
bool should_drop_inbound_header(const std::string_view name) {
    ...
    return core::text::starts_with(lower, "x-forwarded-") || core::text::starts_with(lower, "cf-") ||
           core::text::starts_with(lower, "x-tightrope-");
}
...
HeaderMap build_upstream_headers(
    ...
    erase_header_case_insensitive(headers, "chatgpt-account-id");
    erase_header_case_insensitive(headers, "cookie");
...
HeaderMap build_upstream_websocket_headers(
    ...
    ensure_responses_websocket_beta_header(headers);
    return headers;
}
