...
constexpr std::string_view kMemoriesTraceSummarizePath = "/codex/memories/trace_summarize";
...
std::string normalize_memories_trace_summarize_request(const std::string& raw_request_body) {
    glz::generic payload;
    if (const auto ec = glz::read_json(payload, raw_request_body); ec || !payload.is_object()) {
        throw std::runtime_error("memories request payload is not valid JSON");
    }
    const auto serialized = glz::write_json(payload);
    if (!serialized) {
        throw std::runtime_error("failed to serialize memories request payload");
    }
...
UpstreamRequestPlan build_memories_trace_summarize_http_request_plan(
    const std::string& raw_request_body,
    const HeaderMap& inbound_headers,
    const std::string_view access_token,
    const std::string_view account_id,
    ...
    const auto normalized_body = normalize_memories_trace_summarize_request(raw_request_body);
    const auto prepared_inbound = prepare_inbound(inbound_headers);

    return UpstreamRequestPlan{
        .method = "POST",
        .path = std::string(kMemoriesTraceSummarizePath),
        .transport = "http-json",
