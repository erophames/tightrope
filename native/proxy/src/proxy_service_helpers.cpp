...
std::string format_credit_balance(const std::string& balance) {
    ...
    double numeric = 0.0;
    const auto [ptr, ec] = std::from_chars(trimmed.data(), trimmed.data() + trimmed.size(), numeric);
    if (ec != std::errc{}) {
        return trimmed;
    }
    std::ostringstream out;
...
bool is_supported_memories_route(const std::string_view route) {
    return route == "/v1/memories/trace_summarize" || route == "/backend-api/codex/memories/trace_summarize";
}
...
void append_upstream_passthrough_headers(openai::HeaderMap& downstream, const openai::HeaderMap& upstream) {
    const auto append_named = [&](const std::string_view source_name, const std::string_view output_name) {
        if (const auto value = core::text::find_value_case_insensitive(upstream, source_name); value.has_value()) {
            downstream[std::string(output_name)] = std::string(core::text::trim_ascii(*value));
        }
    };

    append_named("x-codex-turn-state", "x-codex-turn-state");
    append_named("x-models-etag", "x-models-etag");
    append_named("x-reasoning-included", "x-reasoning-included");
    append_named("openai-model", "openai-model");
    if (!downstream.contains("openai-model")) {
        append_named("x-openai-model", "openai-model");
    }
}

openai::HeaderMap build_codex_usage_headers_for_account(const std::string_view account_id) {
    const auto normalized_account_id = core::text::trim_ascii(account_id);
    const auto snapshot = usage::cached_usage_payload(account_id);
    if (!snapshot.has_value() && normalized_account_id.empty()) {
        return {};
    }

    openai::HeaderMap headers;
    if (!normalized_account_id.empty()) {
        headers["x-tightrope-routed-account-id"] = normalized_account_id;
    }
