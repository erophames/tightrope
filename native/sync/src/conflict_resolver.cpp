...
enum class RuntimeConflictResolution : std::uint8_t {
    Lww = 0,
    SitePriority = 1,
    FieldMerge = 2,
};

std::atomic<RuntimeConflictResolution>& runtime_conflict_resolution_mode() {
    static std::atomic<RuntimeConflictResolution> mode{RuntimeConflictResolution::Lww};
    return mode;
}

RuntimeConflictResolution parse_runtime_conflict_resolution(const std::string_view strategy) {
    if (strategy == "site_priority") {
        return RuntimeConflictResolution::SitePriority;
    }
    if (strategy == "field_merge") {
        return RuntimeConflictResolution::FieldMerge;
    }
    return RuntimeConflictResolution::Lww;
}

std::string_view runtime_conflict_resolution_name(const RuntimeConflictResolution mode) {
    switch (mode) {
    case RuntimeConflictResolution::SitePriority:
        return "site_priority";
    case RuntimeConflictResolution::FieldMerge:
        return "field_merge";
    case RuntimeConflictResolution::Lww:
    default:
        return "lww";
    }
}
...
ConflictDecision resolve_lww(const Hlc& remote, const Hlc& local) {
    switch (runtime_conflict_resolution_mode().load(std::memory_order_relaxed)) {
    case RuntimeConflictResolution::SitePriority:
        if (remote.site_id != local.site_id) {
            return remote.site_id > local.site_id ? ConflictDecision::ApplyRemote : ConflictDecision::KeepLocal;
        }
        return compare_hlc(remote, local) > 0 ? ConflictDecision::ApplyRemote : ConflictDecision::KeepLocal;
    case RuntimeConflictResolution::FieldMerge:
        if (remote.wall != local.wall) {
            return remote.wall > local.wall ? ConflictDecision::ApplyRemote : ConflictDecision::KeepLocal;
        }
        if (remote.counter != local.counter) {
            return remote.counter > local.counter ? ConflictDecision::ApplyRemote : ConflictDecision::KeepLocal;
        }
...
void set_runtime_conflict_resolution(const std::string_view strategy) {
    runtime_conflict_resolution_mode().store(parse_runtime_conflict_resolution(strategy), std::memory_order_relaxed);
}

std::string_view runtime_conflict_resolution() {
    return runtime_conflict_resolution_name(runtime_conflict_resolution_mode().load(std::memory_order_relaxed));
}
