...
std::vector<nuraft_backend::ServerInfo> RaftNode::servers() const {
    if (backend_ == nullptr) {
        std::vector<nuraft_backend::ServerInfo> fallback;
        fallback.reserve(membership_.members().size());
        for (const auto member_id : membership_.members()) {
            if (member_id == 0) {
                continue;
            }
            fallback.push_back(nuraft_backend::ServerInfo{
                .node_id = member_id,
                .endpoint = nuraft_backend::endpoint_for(member_id, port_base_),
            });
        }
        return fallback;
    }
    return backend_->servers();
}
