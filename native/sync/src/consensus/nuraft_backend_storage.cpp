...
bool SqliteRaftStorage::open() {
    ...
    const auto plaintext_database = db::connection::database_file_looks_plaintext_sqlite(path_);
    if (plaintext_database && db::connection::has_session_passphrase() &&
        !db::connection::migrate_plaintext_database_with_sqlcipher(path_, &last_error_)) {
        close_locked();
        return false;
    }
    ...
    if (!db::connection::apply_session_key(db_, &last_error_)) {
        close_locked();
        return false;
    }
