...
std::mutex& auto_compact_override_mutex() {
    static std::mutex mutex;
    return mutex;
}

std::optional<std::uint64_t>& auto_compact_retention_override_storage() {
...
AutoCompactConfig load_auto_compact_config() {
    ...
    {
        std::lock_guard<std::mutex> lock(auto_compact_override_mutex());
        const auto override_retention = auto_compact_retention_override_storage();
        if (override_retention.has_value()) {
            config.retention_ms = *override_retention;
        }
    }
    return config;
}
...
void set_auto_compact_retention_override_ms(const std::optional<std::uint64_t> retention_ms) {
    std::lock_guard<std::mutex> lock(auto_compact_override_mutex());
    auto_compact_retention_override_storage() = retention_ms;
}

std::optional<std::uint64_t> auto_compact_retention_override_ms() {
    std::lock_guard<std::mutex> lock(auto_compact_override_mutex());
    return auto_compact_retention_override_storage();
}
