...
std::string request_log_json(const controllers::RequestLogPayload& log) {
    return std::string(R"({"id":)") + std::to_string(log.id) + R"(,"accountId":)" +
           ...
           R"(,"routingStrategy":)" + optional_string_json(log.routing_strategy) +
           R"(,"sticky":)" + bool_json(log.sticky) +
           R"(,"routingScore":)" + optional_double_json(log.routing_score) +
           R"(,"totalCost":)" + number_json(log.total_cost) +
           R"(,"latencyMs":)" + optional_int64_json(log.latency_ms) +
           R"(,"totalTokens":)" + optional_int64_json(log.total_tokens) + "}";
}
