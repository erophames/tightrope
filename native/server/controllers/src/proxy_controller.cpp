...
ProxyHttpResponse post_proxy_responses_json(
    ...
    return {
        ...
        .sticky_reused = result.sticky_reused,
    };
}
...
ProxyWsResponse proxy_responses_websocket(
    ...
    const auto result = proxy::proxy_responses_websocket(route, body, headers, binary_frame);
    return {
        .status = result.status,
        .accepted = result.accepted,
        .close_code = result.close_code,
        .frames = result.frames,
        .routed_account_id = result.routed_account_id,
        .sticky_reused = result.sticky_reused,
    };
}
...
ProxyHttpResponse post_proxy_memories_trace_summarize(
    const std::string_view route,
    const std::string& body,
    const proxy::openai::HeaderMap& headers
) {
    const auto result = proxy::collect_memories_trace_summarize(route, body, headers);
    return {
        .status = result.status,
        .content_type = "application/json",
        .body = result.body,
