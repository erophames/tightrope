...
public:
    ConflictResolutionGuard() : previous_(tightrope::sync::runtime_conflict_resolution()) {}
    ~ConflictResolutionGuard() {
        tightrope::sync::set_runtime_conflict_resolution(previous_);
    }
...
TEST_CASE("conflict resolver LWW applies newer remote HLC", "[sync][conflict]") {
    ConflictResolutionGuard guard;
    tightrope::sync::set_runtime_conflict_resolution("lww");
...
TEST_CASE("conflict resolver site_priority mode prefers higher site id", "[sync][conflict]") {
    ConflictResolutionGuard guard;
    tightrope::sync::set_runtime_conflict_resolution("site_priority");

    const tightrope::sync::Hlc remote = {.wall = 100, .counter = 1, .site_id = 9};
    const tightrope::sync::Hlc local = {.wall = 900, .counter = 9, .site_id = 2};

    REQUIRE(tightrope::sync::resolve_lww(remote, local) == tightrope::sync::ConflictDecision::ApplyRemote);
    REQUIRE(tightrope::sync::runtime_conflict_resolution() == std::string_view("site_priority"));
}

TEST_CASE("conflict resolver field_merge mode keeps local on exact timestamp ties", "[sync][conflict]") {
    ConflictResolutionGuard guard;
    tightrope::sync::set_runtime_conflict_resolution("field_merge");

    const tightrope::sync::Hlc remote = {.wall = 500, .counter = 7, .site_id = 9};
    const tightrope::sync::Hlc local = {.wall = 500, .counter = 7, .site_id = 1};

    REQUIRE(tightrope::sync::resolve_lww(remote, local) == tightrope::sync::ConflictDecision::KeepLocal);
    REQUIRE(tightrope::sync::runtime_conflict_resolution() == std::string_view("field_merge"));
}
