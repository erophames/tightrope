...
TEST_CASE("settings controller returns defaults and supports updates", "[server][settings]") {
    ...
    REQUIRE(initial.settings.routing_strategy == "weighted_round_robin");
    REQUIRE_FALSE(initial.settings.strict_lock_pool_continuations);
    REQUIRE(initial.settings.locked_routing_account_ids.empty());
    ...
    patch.strict_lock_pool_continuations = true;
    patch.locked_routing_account_ids = std::vector<std::string>{"acc-a", "acc-b", "acc-a"};
    ...
    REQUIRE(updated.settings.strict_lock_pool_continuations);
    REQUIRE(updated.settings.locked_routing_account_ids.size() == 2);
    REQUIRE(updated.settings.locked_routing_account_ids[0] == "acc-a");
    REQUIRE(updated.settings.locked_routing_account_ids[1] == "acc-b");
...
TEST_CASE("settings controller preserves sticky toggles when omitted from update", "[server][settings]") {
    const auto db_path = make_temp_db_path();
    sqlite3* db = nullptr;
    REQUIRE(sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK);
    REQUIRE(db != nullptr);
    ...
    tightrope::server::controllers::DashboardSettingsUpdate initial_patch;
    initial_patch.sticky_threads_enabled = true;
    initial_patch.prefer_earlier_reset_accounts = true;
    const auto initial = tightrope::server::controllers::update_settings(initial_patch, db);
    REQUIRE(initial.status == 200);
    REQUIRE(initial.settings.sticky_threads_enabled);
    REQUIRE(initial.settings.prefer_earlier_reset_accounts);

    tightrope::server::controllers::DashboardSettingsUpdate partial_patch;
    partial_patch.theme = "dark";
    const auto updated = tightrope::server::controllers::update_settings(partial_patch, db);
    REQUIRE(updated.status == 200);
    REQUIRE(updated.settings.theme == "dark");
    REQUIRE(updated.settings.sticky_threads_enabled);
    REQUIRE(updated.settings.prefer_earlier_reset_accounts);
...
TEST_CASE("settings controller changes database passphrase for active session", "[server][settings]") {
    const auto db_path = make_temp_db_path();
    tightrope::db::SqlitePool pool(db_path);
    struct CleanupGuard {
        std::string path;
        tightrope::db::SqlitePool* pool = nullptr;
        ~CleanupGuard() {
            tightrope::auth::crypto::clear_token_storage_session_passphrase();
    ...
    CleanupGuard cleanup{db_path, &pool};
    tightrope::db::connection::set_session_passphrase("current-passphrase");
    tightrope::auth::crypto::configure_token_storage_session_passphrase("current-passphrase");
    REQUIRE(pool.open());
    sqlite3* db = pool.connection();
    REQUIRE(db != nullptr);
    REQUIRE(tightrope::db::run_migrations(db));

    const auto invalid = tightrope::server::controllers::change_database_passphrase(
        "wrong-passphrase",
        "next-passphrase",
        db
    );
    REQUIRE(invalid.status == 401);
    REQUIRE(invalid.code == "invalid_current_passphrase");

    const auto updated = tightrope::server::controllers::change_database_passphrase(
        "current-passphrase",
        "next-passphrase",
        db
    );
    INFO("change_database_passphrase code=" << updated.code << " message=" << updated.message);
    REQUIRE(updated.status == 200);
    REQUIRE(tightrope::db::connection::session_passphrase_matches("next-passphrase"));
}
