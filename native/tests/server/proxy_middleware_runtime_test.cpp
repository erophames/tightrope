...
#if TIGHTROPE_HAS_ZSTD_DECOMPRESSION
std::string zstd_compress_body(const std::string& body) {
    const auto bound = ZSTD_compressBound(body.size());
    std::string compressed(bound, '\0');
    const auto written = ZSTD_compress(compressed.data(), compressed.size(), body.data(), body.size(), 3);
...
SELECT path, method, status_code, model, error_code, transport, routing_strategy, account_id
FROM request_logs
ORDER BY id DESC
LIMIT 1;
)SQL";
    ...
    if (const auto* value = sqlite3_column_text(stmt, 6); value != nullptr) {
        snapshot.routing_strategy = std::string(reinterpret_cast<const char*>(value));
    }
    if (sqlite3_column_type(stmt, 7) != SQLITE_NULL) {
        snapshot.account_id = sqlite3_column_int64(stmt, 7);
    }
...
TEST_CASE("runtime validates content encoding and propagates request id", "[server][runtime][proxy][middleware]") {
...
#if TIGHTROPE_HAS_ZSTD_DECOMPRESSION
    const auto zstd_payload = zstd_compress_body(plain_payload);
    const auto zstd_ok = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
                ...
                {"X-Request-Id", "req-zstd-001"},
            },
            zstd_payload
        )
    );
    REQUIRE(zstd_ok.find("200 OK") != std::string::npos);
    REQUIRE(tightrope::tests::server::http_header_value(zstd_ok, "x-request-id").value_or("") == "req-zstd-001");
    ++expected_calls;
    REQUIRE(fake->call_count == expected_calls);
    ...
    REQUIRE(fake->last_plan.headers.at("x-request-id") == "req-zstd-001");

    const auto zstd_with_params = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
                {"Content-Type", "application/json"},
                {"Content-Encoding", "zstd; level=3"},
                {"X-Request-Id", "req-zstd-param-001"},
            },
            zstd_payload
        )
    );
    REQUIRE(zstd_with_params.find("200 OK") != std::string::npos);
    REQUIRE(
        tightrope::tests::server::http_header_value(zstd_with_params, "x-request-id").value_or("") ==
        "req-zstd-param-001");
    ++expected_calls;
    REQUIRE(fake->call_count == expected_calls);
    REQUIRE(fake->last_plan.body.find("\"model\":\"gpt-5.4\"") != std::string::npos);
    REQUIRE(fake->last_plan.headers.find("x-request-id") != fake->last_plan.headers.end());
    REQUIRE(fake->last_plan.headers.at("x-request-id") == "req-zstd-param-001");

    const auto zstd_without_header = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
                {"Content-Type", "application/json"},
                {"X-Request-Id", "req-zstd-sniff-001"},
            },
            zstd_payload
        )
    );
    REQUIRE(zstd_without_header.find("200 OK") != std::string::npos);
    REQUIRE(
        tightrope::tests::server::http_header_value(zstd_without_header, "x-request-id").value_or("") ==
        "req-zstd-sniff-001");
    ++expected_calls;
    REQUIRE(fake->call_count == expected_calls);
    REQUIRE(fake->last_plan.body.find("\"model\":\"gpt-5.4\"") != std::string::npos);
    REQUIRE(fake->last_plan.headers.find("x-request-id") != fake->last_plan.headers.end());
    REQUIRE(fake->last_plan.headers.at("x-request-id") == "req-zstd-sniff-001");
#endif
...
TEST_CASE("runtime proxy stop gate blocks proxy routing until restarted", "[server][runtime][proxy][lifecycle]") {
    const auto db_path = make_temp_db_path();
    sqlite3* db = nullptr;
    REQUIRE(sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK);
    REQUIRE(db != nullptr);
    ...
    REQUIRE(denied.find("503 Service Unavailable") != std::string::npos);
    REQUIRE(denied.find("\"code\":\"proxy_disabled\"") != std::string::npos);
    REQUIRE(fake->call_count == 0);

    const auto started = tightrope::tests::server::send_raw_http(
        port,
        "POST /api/runtime/proxy/start HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Length: 0\r\nConnection: close\r\n\r\n"
    );
    REQUIRE(started.find("200 OK") != std::string::npos);
    REQUIRE(started.find("\"enabled\":true") != std::string::npos);
...
TEST_CASE("runtime compact responses include reference-upstream style rate-limit headers", "[server][runtime][proxy][headers]") {
    const auto db_path = make_temp_db_path();
    sqlite3* db = nullptr;
    REQUIRE(sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK);
    REQUIRE(db != nullptr);
...
TEST_CASE(
    "runtime response continuity does not auto-inject previous_response_id across API keys",
    "[server][runtime][proxy][policy][continuity]"
) {
    const auto db_path = make_temp_db_path();
    sqlite3* db = nullptr;
    REQUIRE(sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK);
    ...
    tightrope::server::controllers::ApiKeyCreateRequest key_a_request;
    key_a_request.name = "Continuity Key A";
    const auto key_a = tightrope::server::controllers::create_api_key(key_a_request, db);
    REQUIRE(key_a.status == 201);
    REQUIRE_FALSE(key_a.key.empty());

    tightrope::server::controllers::ApiKeyCreateRequest key_b_request;
    key_b_request.name = "Continuity Key B";
    const auto key_b = tightrope::server::controllers::create_api_key(key_b_request, db);
    REQUIRE(key_b.status == 201);
    REQUIRE_FALSE(key_b.key.empty());
    sqlite3_close(db);
    ...
    auto fake = std::make_shared<tightrope::tests::proxy::FakeUpstreamTransport>(
        [&observed_plans](const tightrope::proxy::openai::UpstreamRequestPlan& plan) {
            ...
            if (call_index == 1) {
                return tightrope::proxy::UpstreamExecutionResult{
                    .status = 200,
                    .body = R"({"id":"resp_runtime_scope_a_1","object":"response","status":"completed","output":[]})",
                };
            }
            if (call_index == 2) {
                return tightrope::proxy::UpstreamExecutionResult{
                    .status = 200,
                    .body = R"({"id":"resp_runtime_scope_b_1","object":"response","status":"completed","output":[]})",
                };
            }
            return tightrope::proxy::UpstreamExecutionResult{
                .status = 200,
                .body = R"({"id":"resp_runtime_scope_a_2","object":"response","status":"completed","output":[]})",
            };
        }
    );
    tightrope::tests::proxy::ScopedUpstreamTransport scoped_transport(fake);
    ...
    const std::string payload = R"({"model":"gpt-5.4","input":"scope continuity"})";
    const auto first = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
                {"Content-Type", "application/json"},
                {"Authorization", "Bearer " + key_a.key},
                {"session_id", "runtime-continuity-shared"},
            },
            payload
        )
    );
    const auto second = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
                {"Content-Type", "application/json"},
                {"Authorization", "Bearer " + key_b.key},
                {"session_id", "runtime-continuity-shared"},
            },
            payload
        )
    );
    const auto third = tightrope::tests::server::send_raw_http(
        port,
        make_http_post_request(
            "/v1/responses",
            {
    ...
    REQUIRE(third.find("200 OK") != std::string::npos);
...
TEST_CASE("runtime persists successful responses request logs with http transport", "[server][runtime][proxy][request-log]") {
    ...
    REQUIRE(response.find("200 OK") != std::string::npos);
    REQUIRE(
        tightrope::tests::server::http_header_value(response, "x-tightrope-routed-account-id") ==
        std::optional<std::string>{"acc-log-001"}
    );
    ...
    REQUIRE(logged->routing_strategy.has_value());
    REQUIRE(*logged->routing_strategy == "weighted_round_robin");
