...
public:
    explicit LinearizableGuardOverride(std::optional<bool> allow, std::optional<std::string> leader_id = std::nullopt) {
...
TEST_CASE("proxy API key middleware allows valid key even when node is follower", "[server][middleware][api-key][linearizable]") {
    const auto db_path = make_temp_db_path();
    sqlite3* db = nullptr;
    REQUIRE(sqlite3_open_v2(db_path.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr) == SQLITE_OK);
    REQUIRE(db != nullptr);
    ...
    tightrope::server::controllers::ApiKeyCreateRequest create_request;
    create_request.name = "Follower key";
    const auto created = tightrope::server::controllers::create_api_key(create_request, db);
    REQUIRE(created.status == 201);
    REQUIRE_FALSE(created.key.empty());
    ...
    LinearizableGuardOverride guard(/*allow=*/false, "2");
    const auto decision =
        tightrope::server::middleware::validate_proxy_api_key_request(db, "/backend-api/codex/responses", headers);
    REQUIRE(decision.allow);
    REQUIRE(decision.status == 200);
