...
TEST_CASE("bridge cluster_enable rejects peer networking when TLS is disabled", "[bridge][lifecycle][cluster][security]") {
    tightrope::bridge::Bridge bridge;
    REQUIRE(bridge.init(next_bridge_config()));
    ...
    REQUIRE(bridge.last_error().find("tls_enabled=true") != std::string::npos);
}
