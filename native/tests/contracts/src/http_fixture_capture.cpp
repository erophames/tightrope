...
std::string fixture_json_text(const CapturedHttpFixture& fixture) {
    ...
    out << "  \"source\": \"upstream-source-contracts\",\n";
    out << "  \"request\": {\n";
...
std::string manifest_json_text(
    ...
    out << "  \"capture_mode\": \"source-derived-from-reference\",\n";
...
std::vector<CapturedHttpFixture> capture_http_contract_fixtures(
    ...
    for (const auto& seed : seed_fixtures) {
        ...
        if (!source.has_value()) {
            throw std::runtime_error("Seed route not found in reference-upstream source: " + seed.method + " " + seed.path);
        }
