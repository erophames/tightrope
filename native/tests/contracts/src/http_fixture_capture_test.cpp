...
TEST_CASE("http contract capture derives fixture metadata from reference-upstream source", "[contracts][capture]") {
    const auto reference_root = reference_repo_root();
    if (reference_root.empty()) {
        SKIP("REFERENCE_REPO_ROOT not configured for contract capture");
    }
    ...
    const auto fixtures =
        tightrope::tests::contracts::capture_http_contract_fixtures(reference_root, temp_root / "manifest.json");
    ...
    tightrope::tests::contracts::write_captured_http_contract_fixtures(fixtures, temp_root, reference_root);

    const auto manifest = tightrope::tests::contracts::read_text_file(temp_root / "manifest.json");
    REQUIRE(manifest.find("\"capture_mode\": \"source-derived-from-reference\"") != std::string::npos);
}
