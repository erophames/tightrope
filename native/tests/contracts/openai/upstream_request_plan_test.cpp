...
TEST_CASE("responses HTTP plan mirrors reference-upstream upstream path and payload shaping", "[proxy][provider]") {
...
TEST_CASE("compact HTTP plan matches codex compact request shape", "[proxy][provider]") {
    ...
    REQUIRE(plan.body.find("\"tools\":[]") != std::string::npos);
    REQUIRE(plan.body.find("\"parallel_tool_calls\":false") != std::string::npos);
    REQUIRE(plan.body.find("\"service_tier\"") == std::string::npos);
    REQUIRE(plan.body.find("\"prompt_cache_key\"") == std::string::npos);
    REQUIRE(plan.headers.find("Accept") != plan.headers.end());
...
TEST_CASE("memories summarize HTTP plan targets codex memories endpoint", "[proxy][provider]") {
    const std::string payload = R"({"input":[{"id":"m-1","content":"remember this"}]})";
    const tightrope::proxy::openai::HeaderMap inbound = {
        {"User-Agent", "codex-cli"},
        {"X-OpenAI-Client-ID", "client-1"},
    };

    const auto plan = tightrope::proxy::openai::build_memories_trace_summarize_http_request_plan(
        payload,
        inbound,
        "access-token",
        "acc-memory-1",
    ...
    REQUIRE(plan.method == "POST");
    REQUIRE(plan.path == "/codex/memories/trace_summarize");
    REQUIRE(plan.transport == "http-json");
    REQUIRE(plan.body.find("\"input\"") != std::string::npos);
    REQUIRE(plan.headers.find("Accept") != plan.headers.end());
    REQUIRE(plan.headers.at("Accept") == "application/json");
    REQUIRE(plan.headers.find("Content-Type") != plan.headers.end());
    REQUIRE(plan.headers.at("Content-Type") == "application/json");
    REQUIRE(plan.headers.find("Authorization") != plan.headers.end());
    REQUIRE(plan.headers.at("Authorization") == "Bearer access-token");
    REQUIRE(plan.headers.find("chatgpt-account-id") != plan.headers.end());
    REQUIRE(plan.headers.at("chatgpt-account-id") == "acc-memory-1");
}

TEST_CASE("memories summarize plan rejects invalid JSON payload", "[proxy][provider]") {
    REQUIRE_THROWS(
        tightrope::proxy::openai::build_memories_trace_summarize_http_request_plan(
            "not json",
            {},
            "access-token",
            "acc-memory-1",
...
TEST_CASE("transcribe HTTP plan uses minimal reference-upstream transcribe forwarding contract", "[proxy][provider]") {
    const tightrope::proxy::openai::HeaderMap inbound = {
        {"User-Agent", "codex-cli"},
