...
std::string normalize_conflict_resolution(std::string_view strategy) {
    const auto normalized = core::text::to_lower_ascii(core::text::trim_ascii(strategy));
    if (normalized == "site_priority") {
        return "site_priority";
    }
    if (normalized == "field_merge") {
        return "field_merge";
    }
    return "lww";
}
...
std::optional<std::string> discovery_advertise_host(const BridgeConfig& bridge_config) {
    ...
    const auto host_name = boost::asio::ip::host_name(error);
    if (error || host_name.empty()) {
        return std::nullopt;
    }

    boost::asio::io_context io_context;
    boost::asio::ip::tcp::resolver resolver(io_context);
    const auto results = resolver.resolve(host_name, "", error);
    if (error) {
        return std::nullopt;
    }
    ...
    for (const auto& entry : results) {
        ...
        const auto candidate = address.to_string();
        if (candidate.empty() || is_loopback_or_unspecified_host(candidate)) {
            continue;
        }
        return candidate;
    }

    // Fallback for environments where hostname->address resolution fails but a
    // routable local interface still exists (common on dev laptops).
    boost::asio::io_context udp_io_context;
    boost::asio::ip::udp::resolver udp_resolver(udp_io_context);
    const auto udp_targets = udp_resolver.resolve(boost::asio::ip::udp::v4(), "8.8.8.8", "53", error);
    if (!error) {
        boost::asio::ip::udp::socket udp_socket(udp_io_context);
        udp_socket.open(boost::asio::ip::udp::v4(), error);
        if (!error) {
            const auto target = udp_targets.begin();
            if (target != udp_targets.end()) {
                udp_socket.connect(*target, error);
                if (!error) {
                    const auto endpoint = udp_socket.local_endpoint(error);
                    if (!error) {
                        const auto candidate = endpoint.address().to_string();
                        if (!candidate.empty() && !is_loopback_or_unspecified_host(candidate)) {
                            return candidate;
                        }
                    }
...
std::optional<std::string> raft_endpoint_from_sync_endpoint(
    const sync::discovery::PeerEndpoint& endpoint,
    const std::uint32_t site_id
) {
    if (site_id == 0 || endpoint.port == 0) {
        return std::nullopt;
    }
    const auto raft_port = static_cast<std::uint32_t>(endpoint.port) + site_id;
    if (raft_port == 0 || raft_port > 65535U) {
        return std::nullopt;
    }
    return endpoint.host + ":" + std::to_string(raft_port);
}

std::optional<sync::discovery::PeerEndpoint> sync_endpoint_from_raft_endpoint(
    const std::string_view raft_endpoint,
    const std::uint32_t site_id
) {
    if (site_id == 0) {
        return std::nullopt;
    }
    const auto parsed = sync::discovery::parse_endpoint(raft_endpoint);
    if (!parsed.has_value() || parsed->port <= site_id) {
        return std::nullopt;
    }
    const auto sync_port = static_cast<std::uint32_t>(parsed->port) - site_id;
    if (sync_port == 0 || sync_port > 65535U) {
        return std::nullopt;
    }
    return sync::discovery::PeerEndpoint{
        .host = parsed->host,
        .port = static_cast<std::uint16_t>(sync_port),
    };
}
...
bool has_observed_raft_leader(sync::consensus::RaftNode& raft) {
    const auto& state = raft.state();
    return state.role == sync::consensus::RaftRole::Leader || state.leader_id > 0;
}

bool wait_for_observed_raft_leader(sync::consensus::RaftNode& raft, const std::chrono::milliseconds timeout) {
    const auto deadline = std::chrono::steady_clock::now() + timeout;
    while (std::chrono::steady_clock::now() < deadline) {
        if (has_observed_raft_leader(raft)) {
            return true;
        }
        std::this_thread::sleep_for(kRaftPollInterval);
    }
    return has_observed_raft_leader(raft);
}

bool should_attempt_stale_raft_bootstrap_recovery(const ClusterConfig& config) {
    // Discovery-only bootstrap can still start with no peers visible yet.
    // In that mode stale local NuRaft metadata can strand the node as candidate.
    return config.manual_peers.empty();
}

std::string resolve_runtime_db_path(const BridgeConfig& bridge_config) {
    config::ConfigOverrides overrides{};
    if (!bridge_config.db_path.empty()) {
        overrides.db_path = bridge_config.db_path;
    }
    if (!bridge_config.config_path.empty()) {
        overrides.config_path = bridge_config.config_path;
    }

    const auto resolved = config::load_config(overrides);
    if (!resolved.db_path.empty()) {
        return resolved.db_path;
    }
    return "store.db";
}

std::string raft_storage_base_dir(const BridgeConfig& bridge_config) {
    if (bridge_config.db_path.empty()) {
        return {};
    }
    return std::filesystem::path(bridge_config.db_path).parent_path().string();
}
...
std::filesystem::path raft_storage_path_for_node(
    ...
    std::error_code ec;
    const auto base_dir = raft_storage_base_dir(bridge_config);
    auto root = base_dir.empty()
    ...
    root /= "tightrope";
    root /= "raft";
    const auto filename = "nuraft-" + std::to_string(sync_port) + "-" + std::to_string(site_id) + ".db";
...
bool Bridge::init(const BridgeConfig& config) noexcept {
    ...
    if (config.db_passphrase.empty()) {
        set_last_error("database passphrase is required");
        core::logging::log_event(
            core::logging::LogLevel::Error,
            "runtime",
            "bridge",
            "init_failed",
            "reason=db_passphrase_missing"
        );
        return false;
    }

    db::connection::set_session_passphrase(config.db_passphrase);
    auth::crypto::configure_token_storage_session_passphrase(config.db_passphrase);

    const auto runtime_db_path = resolve_runtime_db_path(config);
    db::SqlitePool db_pool(runtime_db_path);
    if (!db_pool.open()) {
        auth::crypto::clear_token_storage_session_passphrase();
        db::connection::clear_session_passphrase();
        const auto db_error = db_pool.last_error();
        if (!db_error.empty()) {
            set_last_error("database unlock failed: " + db_error);
        } else {
            set_last_error("database unlock failed");
        }
        core::logging::log_event(
            core::logging::LogLevel::Error,
            "runtime",
            "bridge",
            "init_failed",
            db_error.empty() ? "reason=db_unlock_failed" : "reason=db_unlock_failed error=" + db_error
        );
        return false;
    }
    db_pool.close();
...
bool Bridge::cluster_enable(const ClusterConfig& config) noexcept {
    ...
    if (config.journal_retention_days == 0 || config.journal_retention_days > 3650) {
        set_last_error("journal_retention_days must be between 1 and 3650");
        core::logging::log_event(
            core::logging::LogLevel::Warning,
            "runtime",
            "bridge",
            "cluster_enable_rejected",
            "reason=invalid_journal_retention_days"
        );
        return false;
    }
    ...
    resolved_config.conflict_resolution = normalize_conflict_resolution(resolved_config.conflict_resolution);
    ...
    sync::set_runtime_conflict_resolution(resolved_config.conflict_resolution);
    constexpr std::uint64_t kMsPerDay = 24ULL * 60ULL * 60ULL * 1000ULL;
    sync::set_auto_compact_retention_override_ms(
        static_cast<std::uint64_t>(resolved_config.journal_retention_days) * kMsPerDay
    );
    ...
    if (secure_peer_networking && (!resolved_config.tls_enabled || !resolved_config.tls_verify_peer)) {
        core::logging::log_event(
            core::logging::LogLevel::Warning,
            "runtime",
            "bridge",
            "cluster_peer_security_weakened",
            "tls_enabled=" + std::string(resolved_config.tls_enabled ? "1" : "0") +
                " tls_verify_peer=" + std::string(resolved_config.tls_verify_peer ? "1" : "0")
        );
    }
    ...
    const auto storage_base_dir = raft_storage_base_dir(config_);
    auto make_raft_node = [&]() {
        return std::make_unique<sync::consensus::RaftNode>(
            resolved_config.site_id,
            std::vector<std::uint32_t>{},
            resolved_config.sync_port,
            storage_base_dir,
            backend_options);
    };
    ...
    auto leader_observed = wait_for_observed_raft_leader(*cluster.raft_node, kRaftElectionTimeout);
    const auto can_attempt_recovery = should_attempt_stale_raft_bootstrap_recovery(resolved_config);
    if (!leader_observed && can_attempt_recovery) {
        const auto stale_storage_path =
            raft_storage_path_for_node(config_, resolved_config.site_id, resolved_config.sync_port);
        std::error_code remove_ec;
        const auto removed_stale_state = std::filesystem::remove(stale_storage_path, remove_ec);
        core::logging::log_event(
            remove_ec ? core::logging::LogLevel::Warning : core::logging::LogLevel::Info,
            "runtime",
            "bridge",
            remove_ec ? "cluster_recover_stale_raft_state_failed" : "cluster_recover_stale_raft_state",
            "path=" + stale_storage_path.string() +
                " removed=" + std::string(removed_stale_state ? "1" : "0") +
                (remove_ec ? " error=" + remove_ec.message() : std::string{}));

        cluster.raft_node->stop();
        cluster.raft_node = make_raft_node();
        if (!cluster.raft_node->start()) {
            set_last_error("raft node failed to start after stale-state recovery");
            core::logging::log_event(
                core::logging::LogLevel::Error,
                "runtime",
                "bridge",
                "cluster_enable_failed",
                "reason=raft_recovery_start_failed");
            return false;
        }
        cluster.raft_node->start_election();
        leader_observed = wait_for_observed_raft_leader(*cluster.raft_node, kRaftElectionTimeout);
    }
    if (!leader_observed && can_attempt_recovery) {
        cluster.raft_node->stop();
        set_last_error("raft leader unavailable and no bootstrap peers are configured");
        core::logging::log_event(
            core::logging::LogLevel::Error,
            "runtime",
            "bridge",
            "cluster_enable_failed",
            "reason=raft_leader_unavailable_no_bootstrap_peers");
        return false;
    }
    ...
    ingress_server_config.local_site_id = resolved_config.site_id;
    ...
    core::logging::log_event(
        ...
        "cluster=" + resolved_config.cluster_name + " site_id=" + std::to_string(resolved_config.site_id) +
            ...
            " conflict_resolution=" + resolved_config.conflict_resolution +
            " journal_retention_days=" + std::to_string(resolved_config.journal_retention_days)
    );
...
bool Bridge::cluster_add_peer(const std::string& address) noexcept {
    ...
    if (!cluster_->config.tls_enabled || !cluster_->config.tls_verify_peer) {
        core::logging::log_event(
            ...
            "runtime",
            "bridge",
            "add_peer_transport_security_weakened",
            "tls_enabled=" + std::string(cluster_->config.tls_enabled ? "1" : "0") +
                " tls_verify_peer=" + std::string(cluster_->config.tls_verify_peer ? "1" : "0")
        );
    }
    ...
    const bool require_probe_success = should_probe_peer_on_add();
    std::optional<std::uint32_t> probed_site_id;
    bool add_probe_ok = false;
    std::optional<std::uint64_t> add_probe_duration_ms;
    std::optional<std::string> add_probe_error;
    {
        const auto probe_policy = read_peer_probe_policy(cluster_->config);
        const auto probe_timeout_ms = require_probe_success
                                        ? probe_policy.timeout_ms
                                        : std::min<std::uint64_t>(probe_policy.timeout_ms, 100);
        const auto probe_config = make_peer_probe_config(cluster_->config, cluster_->config.site_id, 1, probe_timeout_ms);
        const auto probe = sync::transport::probe_peer_handshake(*endpoint, probe_config);
        add_probe_duration_ms = probe.duration_ms;
        if (!probe.ok) {
            add_probe_error = probe.error;
            ...
            core::logging::log_event(
                ...
                "runtime",
                "bridge",
                "add_peer_probe_best_effort_failed",
                "endpoint=" + address + " error=" + probe.error);
        } else {
            add_probe_ok = true;
            probed_site_id = probe.remote_site_id;
        }
    }

    if (probed_site_id.has_value() && *probed_site_id == cluster_->config.site_id) {
        set_last_error("peer endpoint resolves to local site_id: " + std::to_string(*probed_site_id));
        core::logging::log_event(
            core::logging::LogLevel::Warning,
            "runtime",
            "bridge",
            "add_peer_rejected",
            "reason=peer_is_local_site");
        return false;
    }

    const auto proposals = cluster_->peer_manager.membership_proposals();
    if (probed_site_id.has_value()) {
        const auto existing = std::find_if(
            proposals.begin(),
            proposals.end(),
            [&probed_site_id](const sync::discovery::PeerRecord& peer) {
                return peer.site_id == *probed_site_id;
            });
        if (existing != proposals.end()) {
            const auto existing_address = sync::discovery::endpoint_to_string(existing->endpoint);
            if (existing_address != address) {
                set_last_error(
                    "peer site_id conflict: " + std::to_string(*probed_site_id) + " already maps to " + existing_address);
                core::logging::log_event(
                    core::logging::LogLevel::Warning,
                    "runtime",
                    "bridge",
                    "add_peer_rejected",
                    "reason=peer_site_id_conflict");
                return false;
            }
        }
    }
    ...
    if (probed_site_id.has_value()) {
        site_id = *probed_site_id;
    } else {
        std::unordered_set<std::uint32_t> used_site_ids;
        used_site_ids.reserve(proposals.size() + 1);
        used_site_ids.insert(cluster_->config.site_id);
        for (const auto& peer : proposals) {
            used_site_ids.insert(peer.site_id);
        }

        site_id = std::max<std::uint32_t>(1, cluster_->next_peer_site_id);
        while (used_site_ids.contains(site_id)) {
            if (site_id == (std::numeric_limits<std::uint32_t>::max)()) {
                set_last_error("no available site_id for peer");
                core::logging::log_event(
                    core::logging::LogLevel::Warning,
                    "runtime",
                    "bridge",
                    "add_peer_rejected",
                    "reason=site_id_space_exhausted");
                return false;
            }
            ++site_id;
        }
    ...
    const auto raft_endpoint = raft_endpoint_from_sync_endpoint(*endpoint, site_id);
    if (!raft_endpoint.has_value()) {
        (void)cluster_->peer_manager.remove_peer(site_id);
        set_last_error("failed to derive raft endpoint from peer sync endpoint: " + address);
        core::logging::log_event(core::logging::LogLevel::Warning, "runtime", "bridge", "add_peer_rejected", "reason=invalid_raft_endpoint");
        return false;
    }
    std::string raft_error;
    auto apply_membership_change = [&]() {
        raft_error.clear();
        return cluster_->raft_node->add_member(site_id, *raft_endpoint, &raft_error);
    };
    ...
    if (!membership_applied) {
        if (is_retryable_membership_error(raft_error)) {
            if (site_id < (std::numeric_limits<std::uint32_t>::max)()) {
                cluster_->next_peer_site_id =
                    std::max(cluster_->next_peer_site_id, static_cast<std::uint32_t>(site_id + 1));
            }
            if (add_probe_duration_ms.has_value()) {
                cluster_->last_peer_probe_unix_ms[site_id] = now_unix_ms();
                cluster_->peer_probe_last_duration_ms[site_id] = *add_probe_duration_ms;
                if (add_probe_ok) {
                    cluster_->peer_probe_failures[site_id] = 0;
                    cluster_->peer_probe_last_error.erase(site_id);
                } else if (add_probe_error.has_value()) {
                    cluster_->peer_probe_failures[site_id] = 1;
                    cluster_->peer_probe_last_error[site_id] = *add_probe_error;
                }
            }
            clear_last_error();
            core::logging::log_event(
...
void Bridge::refresh_cluster_peers(ClusterStatus& status) noexcept {
    ...
    auto peers = cluster_->peer_manager.membership_proposals();
    ...
    std::unordered_set<std::uint32_t> known_peer_site_ids;
    known_peer_site_ids.reserve(peers.size());
    for (const auto& peer : peers) {
        if (peer.site_id != 0) {
            known_peer_site_ids.insert(peer.site_id);
        }
    }

    if (cluster_->raft_node != nullptr && cluster_->raft_node->is_running()) {
        const auto raft_servers = cluster_->raft_node->servers();
        for (const auto& server : raft_servers) {
            if (server.node_id == 0 || server.node_id == local_site_id || known_peer_site_ids.contains(server.node_id)) {
                continue;
            }
            auto endpoint = sync_endpoint_from_raft_endpoint(server.endpoint, server.node_id);
            if (!endpoint.has_value()) {
                endpoint = sync::discovery::parse_endpoint(server.endpoint);
            }
            if (!endpoint.has_value()) {
                continue;
            }
            peers.push_back(sync::discovery::PeerRecord{
                .site_id = server.node_id,
                .endpoint = *endpoint,
                .manual = true,
                // Do not synthesize heartbeat freshness for raft-membership-only peers.
                // Liveness must come from probe/ingress telemetry, not static membership.
                .seen_unix_ms = 0,
            });
            known_peer_site_ids.insert(server.node_id);
        }
    }
    ...
    struct PeerSiteIdReconciliationCandidate {
        std::uint32_t observed_site_id = 0;
        std::uint32_t remote_site_id = 0;
        std::string address;
    };

    std::vector<std::uint32_t> eviction_candidates;
    std::vector<PeerSiteIdReconciliationCandidate> site_id_reconciliation_candidates;
    ...
    for (const auto& peer : peers) {
        ...
        bool has_ingress_heartbeat = false;
        ...
        if (const auto telemetry = sync::SyncEngine::peer_ingress_telemetry(peer.site_id); telemetry.has_value()) {
            item.last_heartbeat_at = telemetry->last_seen_unix_ms > 0
                                       ...
                                       : std::nullopt;
            has_ingress_heartbeat = item.last_heartbeat_at.has_value();
        ...
        if (probe_policy.enabled && probes_attempted < probe_policy.max_probes_per_refresh) {
            ...
            if (should_probe_now) {
                ...
                if (probe.ok) {
                    ...
                    if (peer.manual && probe.remote_site_id.has_value() && *probe.remote_site_id != 0 &&
                        *probe.remote_site_id != peer.site_id && *probe.remote_site_id != local_site_id) {
                        site_id_reconciliation_candidates.push_back({
                            .observed_site_id = peer.site_id,
                            .remote_site_id = *probe.remote_site_id,
        ...
        // Discovery "seen" timestamps are not transport liveness. Use probe cadence
        // for peers without ingress heartbeat telemetry to avoid connected-state flapping.
        const auto liveness_interval_ms =
            has_ingress_heartbeat
              ? policy.heartbeat_interval_ms
              : std::max<std::uint64_t>(
                    policy.heartbeat_interval_ms,
                    probe_policy.enabled ? probe_policy.interval_ms : policy.heartbeat_interval_ms);
        const auto missed = item.last_heartbeat_at.has_value()
                              ? heartbeat_failures_since(now_ms, *item.last_heartbeat_at, liveness_interval_ms)
                              : 0;
    ...
    if (local_is_leader && !site_id_reconciliation_candidates.empty()) {
        std::unordered_set<std::uint32_t> reconciled_observed_ids;
        for (const auto& candidate : site_id_reconciliation_candidates) {
            if (candidate.observed_site_id == 0 || candidate.remote_site_id == 0 ||
                !reconciled_observed_ids.insert(candidate.observed_site_id).second) {
                continue;
            }
            ...
            core::logging::log_event(
                ...
                "runtime",
                "bridge",
                "peer_site_id_mismatch_detected",
                "observed_site_id=" + std::to_string(candidate.observed_site_id) +
                    " remote_site_id=" + std::to_string(candidate.remote_site_id) +
                    " address=" + candidate.address);

            if (!cluster_remove_peer(std::to_string(candidate.observed_site_id))) {
                const auto error = last_error();
                core::logging::log_event(
                    core::logging::LogLevel::Warning,
                    "runtime",
                    "bridge",
                    "peer_site_id_reconcile_remove_failed",
                    "observed_site_id=" + std::to_string(candidate.observed_site_id) +
                        " remote_site_id=" + std::to_string(candidate.remote_site_id) +
                        " address=" + candidate.address + " error=" + error);
                continue;
            }

            if (!cluster_add_peer(candidate.address)) {
                const auto error = last_error();
                core::logging::log_event(
                    core::logging::LogLevel::Warning,
                    "runtime",
                    "bridge",
                    "peer_site_id_reconcile_add_failed",
                    "observed_site_id=" + std::to_string(candidate.observed_site_id) +
                        " remote_site_id=" + std::to_string(candidate.remote_site_id) +
                        " address=" + candidate.address + " error=" + error);
                continue;
