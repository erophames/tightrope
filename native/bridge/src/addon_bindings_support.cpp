...
BridgeConfig parse_bridge_config(const Napi::Value& value) {
    ...
    if (has_key(object, "db_passphrase")) {
        if (!object.Get("db_passphrase").IsString()) {
            throw Napi::TypeError::New(object.Env(), "init.db_passphrase must be a string");
        }
        config.db_passphrase = object.Get("db_passphrase").As<Napi::String>().Utf8Value();
    }
...
ClusterConfig parse_cluster_config(const Napi::Object& object) {
    ...
    if (has_key(object, "conflict_resolution")) {
        if (!object.Get("conflict_resolution").IsString()) {
            throw Napi::TypeError::New(object.Env(), "clusterEnable.conflict_resolution must be a string");
        }
        const auto strategy = object.Get("conflict_resolution").As<Napi::String>().Utf8Value();
        if (strategy != "lww" && strategy != "site_priority" && strategy != "field_merge") {
            throw Napi::TypeError::New(
                object.Env(),
                "clusterEnable.conflict_resolution must be one of lww, site_priority, field_merge");
        }
        config.conflict_resolution = strategy;
    }
    if (has_key(object, "journal_retention_days")) {
        config.journal_retention_days = require_u32(object, "journal_retention_days", "clusterEnable");
    }
